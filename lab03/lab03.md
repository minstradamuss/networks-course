...
Если узким местом является вторая линия связи ($R_C < R_S$), то второй пакет может находиться во входном буфере перед второй линией, ожидая своей передачи. Это подтверждается следующим расчетом:

Первому пакету, чтобы дойти до маршрутизатора и "выйти" из него, требуется:
...
3. 1) интенсивность трафика (\( B \cdot Δ \)) на канале доступа снижается на 60% (1 - 0.4 = 0.6), значит средняя задержка доступа:Δ'_1 = \frac{0.0567}{1 - 0.4 \cdot 0.9072} = \frac{0.0567}{1 - 0.36288} = \frac{0.0567}{0.63712} \approx 0.09 \ секунд
  2) время ответа:
