...
#### Решение
1) первое время $RTT_{FE}$ уходит на установление соединения между клиентом и внешним сервером

2) далее тратится $RTT_{BE}$ на запрос к серверу и получение ответа от него

3) по условию задачи, окно на внешнем сервере большое, что позволяет получить весь ответ сразу, кроме того, серверу обычно требуется три окна для доставки ответа, что занимает время, равное $3 \cdot RTT_{FE}$ и время на обработку добавляется в конце

4) т.о. общее время на доставку ответа: $4 \cdot RTT_{FE} + RTT_{BE} + \text{ время обработки}$, чтд.
