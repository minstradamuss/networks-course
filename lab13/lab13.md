...
Т.о. максимальное значение достигается при $$p = 1 / N$$

2. Предел эффективности при $$N → ∞$$

Подставляем найденное $$p = 1 / N$$ в исходную формулу:

$$S(p) = N * (1 / N) * (1 - 1 / N)^{(N - 1)} = (1) * (1 - 1 / N)^{(N - 1)}$$

Рассматриваем предел:
$$lim_{N→∞} (1 - 1 / N)^{(N - 1)} = 1/e$$

Т.о. при $$N → ∞$$ максимальная эффективность дискретного протокола ALOHA стремится к $$1 / e ≈ 0.36787$$
...
#### Решение
пусть общее количество узлов: $$N = 4$$, тогда для узла X в произвольном кванте:
- Он должен попытаться передать $$p$$,
- Остальные $$N-1 = 3$$ узла не должны передавать $$(1 - p)^3$$,

$$P_{\text{успех}} = p \cdot (1 - p)^{3}$$

1. Вероятность, что A впервые успешно передаст в кванте 5

Узел A должен:
- не пытаться передавать в квантах 1–4,
- успешно передать в кванте 5.

Вероятность: $$P = (1 - p)^4 \cdot p \cdot (1 - p)^3 = (1 - p)^7 \cdot p$$

2. Вероятность, что Б, В или Г с первого раза передаст в кванте 4

Рассмотрим вероятность для одного конкретного узла (например, Б), что он впервые передаст в 4-м кванте: $$P_{\text{один}} = (1 - p)^3 \cdot p \cdot (1 - p)^3 = (1 - p)^6 \cdot p$$

Три узла: Б, В, Г — события взаимно исключающие, если считаем «один и только один»: $$P_{\text{любой один}} = 3 \cdot (1 - p)^6 \cdot p$$

3. Вероятность, что первая успешная передача в системе произойдет в кванте 3

Условие:
- В квантах 1 и 2 — нет успешных передач,
- В кванте 3 — ровно один успешный узел.

Успех = ровно 1 узел передал, остальные — нет.

Вероятность такой передачи: $$P_{\text{успех}} = \binom{4}{1} \cdot p \cdot (1 - p)^3 = 4p(1 - p)^3$$
