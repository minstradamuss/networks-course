...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще
   ...
   - русский (ru-RU), английский (en-US), китайский (zh-CN - это Chinese (Simplified))
   - браузер; платформа, на которой он запущен с версиями
...
![image](https://github.com/user-attachments/assets/3649e038-f23e-46ce-9bbd-44ed2bf96ebc)
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3
   - 128.119.245.12 (http://gaia.cs.umass.edu/wireshark-labs/HTTP-wireshark-file4.html), 128.119.245.12 (http://gaia.cs.umass.edu/pearson.png), 178.79.137.164 (http://kurose.cslash.net/8E_cover_small.jpg)
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - скорее всего мы можем сказать, что браузер загрузил два изображения последовательно, т.к. сначала были отправлены оба GET-запроса (на картинки) и только потом получен ответ

![image](https://github.com/user-attachments/assets/b566d290-c619-47f2-80ee-34719c2e5749)
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - Authorization, который включает в себя Credentials (с именем пользователя и паролем)

![image](https://github.com/user-attachments/assets/a9125953-ae57-481d-8fe9-58e1d255b257)
